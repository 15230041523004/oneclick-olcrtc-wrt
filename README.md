...
**Версия `0.0.3`.** Однокомандная установка **текущего** [OlcRTC](https://github.com/openlibrecommunity/olcrtc) в режиме **`mode: srv`** на OpenWrt и Debian-family (Debian 12/13, Ubuntu, Raspberry Pi OS и другие системы с systemd). Архитектуры: `amd64`, `arm64`, `armv7`.
...
Нужны Debian 12/13, Ubuntu, Raspberry Pi OS (`ID=debian` или `ID=raspbian`) или другой дистрибутив с `ID_LIKE=debian`, работающий **systemd**, `apt-get`, доступ root или sudo, `x86_64/amd64`, `aarch64/arm64` либо 32-bit `armv7l`, доступ к GitHub и Telemost. Контейнер без systemd этот установщик отклонит до загрузки бинарника. Ориентир по RAM — от 512 МиБ.

Raspberry Pi 3B+ (Cortex-A53, 1 ГиБ): **64-bit** Raspberry Pi OS — путь `arm64` (тот же ELF, что на Debian VDS). **32-bit** Raspberry Pi OS (`uname -m=armv7l`, либо 64-bit ядро + 32-bit userland) ставит `olcrtc-linux-armv7`.
...
2. Роутер: OpenWrt, `uname -m` = `aarch64`, `x86_64` или `armv7l`.
3. RAM:
...
Не используйте `sh -c "$(wget -qO- …)"` — при 404 получится пустой успешный `sh`. Pin на конкретный тег: замените `latest/download` на `download/v0.0.3`.
...
Клиент этого инсталлятора: **[olcbox](https://github.com/alananisimov/olcbox)**. Тот же Room ID и ключ, что на сервере.

Этот инсталлятор не ставит и не настраивает: [owenclave](https://github.com/owenewans/owenclave), [veil](https://github.com/venterum/veil), голый `cnc`.
...
Version `0.0.3`. One-click current [OlcRTC](https://github.com/openlibrecommunity/olcrtc) `mode: srv` on OpenWrt and Debian-family (Debian 12/13, Ubuntu, Raspberry Pi OS, systemd). Architectures: `amd64`, `arm64`, `armv7`.
...
Put `ROOM_ID` on `sh`, not on `wget`/`curl`. That script downloads `olcrtc-linux-arm64`, `olcrtc-linux-amd64`, or `olcrtc-linux-armv7` from the same Release. Do not use `sh -c "$(wget -qO- …)"`. Pin a tag by replacing `latest/download` with `download/v0.0.3`. Phone client: [olcbox](https://github.com/alananisimov/olcbox). owenclave / veil / raw `cnc` are not this installer. Supported RAM floor is **512 MiB** (warning, not an install reject). See [docs/upstream.md](docs/upstream.md).
